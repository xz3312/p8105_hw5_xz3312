...
Across the `r nrow(city_prop_summary)` cities in the dataset, the estimated proportion of unsolved homicides varies widely. The city with the **lowest** estimated proportion is `r city_prop_summary %>%  arrange(estimate) %>%  slice(1) %>%  pull(city_state)` at about `r city_prop_summary %>%  arrange(estimate) %>%  slice(1) %>%  pull(estimate) %>%  round(2)`, while the **highest** occurs in `r city_prop_summary %>%  arrange(desc(estimate)) %>%  slice(1) %>%  pull(city_state)` with an estimate near `r city_prop_summary %>%  arrange(desc(estimate)) %>%  slice(1) %>%  pull(estimate) %>%  round(2)`. The confidence intervals also differ substantially across cities, reflecting differences in sample sizes. Overall, the plot shows that cities with larger unsolved proportions tend to cluster toward the top of the reordered axis, emphasizing substantial variation in clearance rates nationwide.
